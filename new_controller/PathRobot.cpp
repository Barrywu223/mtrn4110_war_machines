...
void PathRobot::readFile()
{
    ...
    {
        std::cout << "Error: unable to find motion plan " << filePath;
        exit(1);
    }
...
void PathRobot::followMP()
{
    ...
    for (int i = 3; i < (int)rawData.length(); i++)
    {
        ...
        // Not sure have to call fuction within the castomRobot
        angleCorrection();
        if (ds[1]->getValue() < 850)
        {
            linearCorrection();
        }
        obstacleDetection();
        std::cout << header << "Step: " << std::setfill('0') << std::setw(3) << step
                  << ", Row: " << x << ", Column: " << y << ", Heading: " << heading << ", Left Wall: "
                  << message.leftWall << ", Front Wall: " << message.frontWall
                  << ", Right Wall: " << message.rightWall << std::endl;
        CSVAppend();
    }
    stop();
