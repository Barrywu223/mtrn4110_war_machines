...
void PathPlanner::run() {
  // Lazy solution to added output requirement
  // Simply runs twice, once for terminal once for file
  findBestPath();
  // Opens the output file and redirects stdout
  FILE* output_file = std::freopen(outputPath.c_str(), "w", stdout);
  // Duplicates opening the file messages as missed
  std::cout << header << "Reading in map from "<< mapPath << "..." << std::endl;
  for (std::string s : rawMap) std::cout << header << s << std::endl;
  std::cout << header << "Map read in!" << std::endl;
  findBestPath();
