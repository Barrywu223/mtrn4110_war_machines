...
Map::Map()
{
    ...
    for (int i = 0; i < MAPSIZE_X; i++)
        map[i] = std::vector<Cell>(MAPSIZE_Y);
    // Make edges
    for (int i = 0; i < MAPSIZE_X; i++)
    {
        map[i][0].up = true;
    ...
    for (int y = start_y; y < start_y + 5; y++)
    {
        for (int x = start_x; x < start_x + 9; x++)
        {
            ...
            // else mapString << ((map[x][y].oor) ? " I " : (map[x][y].explored) ? " 1 " : " 0 ");
                ...
                mapString << ((map[x][y].oor) ? "   " : (map[x][y].explored) ? "   "
                                                                             : "   ");
            if (x == start_x + 9 - 1)
                mapString << ((map[start_x + 9 - 1][y].right) ? "|" : " ");
            else
                mapString << ((map[x][y].right || map[x + 1][y].left) ? "|" : " ");
        }
        mapString << "\n";
        for (int x = start_x; x < start_x + 9; x++)
            mapString << ((map[x][y].down || map[x][y + 1].up) ? " ---" : "    ");
        mapString << "\n";
    }
...
std::string Map::fullMapString()
{
    ...
    for (int y = 0; y < MAPSIZE_Y; y++)
    {
        ...
        {
            if (x == 0)
                mapString << ((map[0][y].left) ? "|" : " ");
            if (x == robot_x && y == robot_y)
            {
                if (robot_heading == 'N')
                    mapString << " ^ ";
                else if (robot_heading == 'S')
                    mapString << " v ";
                else if (robot_heading == 'E')
                    mapString << " > ";
                else if (robot_heading == 'W')
                    mapString << " < ";
            }
            ...
            // else mapString << ((map[x][y].oor) ? " I " : (map[x][y].explored) ? " 1 " : " 0 ");
                ...
                mapString << ((map[x][y].oor) ? "   " : (map[x][y].explored) ? "   "
                                                                             : "   ");
            if (x == MAPSIZE_X - 1)
                mapString << ((map[MAPSIZE_X - 1][y].right) ? "|" : " ");
            else
                mapString << ((map[x][y].right || map[x + 1][y].left) ? "|" : " ");
        }
        mapString << "\n";
...
void ExplorerRobot::explore()
{
    ...
    while (updateMap())
    {
        ...
        std::string path = pp->findBestPath();
        delete pp;
        char move;
        int foundPos;
        if ((foundPos = (int)path.find_first_not_of("0123456789NSEW")) != -1)
            move = path[foundPos];
        else
        {
            stop();
